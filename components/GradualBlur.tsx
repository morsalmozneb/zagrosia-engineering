...
// Inject CSS once — replaces ./GradualBlur.css (not allowed in Next.js components)
...
const useResponsiveDimension = (
  ...
  const [val, setVal] = useState<string | undefined>((config as Record<string, string>)[key as string]);
  useEffect(() => {
  ...
  return responsive ? val : (config as Record<string, string>)[key as string];
};
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      ...
      base[config.position as 'top' | 'bottom'] = 0; base.left = 0; base.right = 0;
    } else if (isHorizontal) {
      base.width = responsiveWidth || responsiveHeight; base.height = '100%';
      base[config.position as 'left' | 'right'] = 0; base.top = 0; base.bottom = 0;
    }
    return base;
